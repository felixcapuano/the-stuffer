...
function Map(props) {
    ...
    const onMarkerClicked = (e) => {
        console.log(selectionMode.current);
    ...
    const setSelectionMode = (_leaflet_id) => {
        console.log(_leaflet_id);
        const mrk = createMarker(getMarkerById(_leaflet_id)).on('click', onMarkerClicked);

        // filtering data by id to keep only relevent one
        const stuffSelection = heatmapData.filter((d) => d.hit_id === _leaflet_id);
